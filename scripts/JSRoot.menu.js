...
JSROOT.define(['d3', 'jquery', 'painter', 'jquery-ui'], (d3, $, jsrp) => {
  ...
  /** @summary Produce exec string for WebCanas to set color value
   ...
   function getColorExec(col, method) {
      ...
      } else if (!Number.isNaN(col) && arr[col]) {
         id = col;
         col = arr[id];
      }
   ...
   class JQueryMenu {
      ...
      addColorMenu(name, value, set_func, fill_kind) {
         ...
         this.add("sub:" + name, function() {
            ...
            if (col === null) return;
            let id = parseInt(col);
            if (!Number.isNaN(id) && jsrp.getColor(id)) {
      ...
      addSizeMenu(name, min, max, step, value, set_func) {
         ...
         this.add("sub:" + name, function() {
            ...
            if (!val) return;
            val = parseFloat(val);
            if (!Number.isNaN(val)) set_func((step >= 1) ? Math.round(val) : val);
         });
      ...
      addAttributesMenu(painter, preffix) {
         ...
         if (painter.lineatt && painter.lineatt.used) {
            ...
            this.add("sub:style", function() {
               ...
               if (id === null) return;
               id = parseInt(id);
               if (Number.isNaN(id) || !jsrp.root_line_styles[id]) return;
